...
const DocumentTile = memo(({ name, ext, base64, localUri }) => {
  ...
  const openDocument = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        'Error',
        'Could not open the file. Make sure a compatible app is installed.',
      );
    } finally {
      setOpening(false);
    }
...
// ─── Image Tile ───────────────────────────────────────────────────────────────
// Existing attachments arrive as a raw Base64 string. Handing that straight to
// <Image> as a `data:` URI forces the JS↔Native bridge to ship the whole
// (often multi-MB) string just to paint a small thumbnail. Writing it to a
...
const ImageTile = memo(({ ext, base64, localUri, cacheKey }) => {
  ...
  const [uri, setUri] = useState(normalizedLocalUri);
  const [loading, setLoading] = useState(!normalizedLocalUri);
  ...
  useEffect(() => {
    ...
    const safeKey = String(cacheKey ?? 'attachment').replace(
      /[^a-zA-Z0-9_-]/g,
      '_',
    );
    const destPath = `${RNBlobUtil.fs.dirs.CacheDir}/thumb_${safeKey}.${extClean}`;
...
const AttachmentImageViewer = memo(({ attachment }) => {
  ...
  const cacheKey = [name, attachment.ATTACHDATE, attachment.ATTACHSIZE]
    .filter(Boolean)
    .join('_');
