...
const CreateCashReceipt = (props) => {
  ...
  const [receiptDate, setReceiptDate] = useState(moment().format());
    ...
    try {
      ...
      if (token) {
        ...
        if (companiesData?.length > 0) {
          setCompany(companiesData[0].Division);
        }
        ...
        if (paymentModeData?.length > 0) {
          setPaymentMode(paymentModeData[0].PaymentMode);
        }
        setLoading(false);
      }
    } catch (e) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await createCashReceipt(userToken, receiptData);
