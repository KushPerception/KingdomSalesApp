...
import { GetCurrentDriveOrderItemCashReceipt } from "../../../utility/ApiHelpers/StagingApis";
...
const DriverCashReceipt = (props) => {
  // Props
  const SLNO = props.route?.params?.SLNO;

  // States
  const [loading, setloading] = useState(true);
  const [UserToken, setUserToken] = useState("");
  const [PdfCashReceipt, setPdfCashReceipt] = useState("");
  ...
  const OrderPdfCallback = (response) => {
    setPdfCashReceipt(response?.data?.pdf);
  ...
  const GetOrderPdf = (UserToken) => {
    setloading(true);
    GetCurrentDriveOrderItemCashReceipt(
      SLNO,
      UserToken,
      OrderPdfCallback,
      setloading
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <View style={styles.container}>
          <Pdf
            source={{
              uri: `${"data:application/pdf;base64,"}${PdfCashReceipt}`,
            }}
...
export default DriverCashReceipt;
