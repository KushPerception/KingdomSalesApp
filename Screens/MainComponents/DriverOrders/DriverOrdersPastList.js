...
import { syncPastDriverOrders } from "../../db/driverOrders/sync";
import NetInfo from "@react-native-community/netinfo";
import { getPastDriverOrdersList } from "../../db/driverOrders/crud";
import moment from "moment";
import { useFocusEffect } from "@react-navigation/native";

/* driver order list */
const DriverOrdersPastList = (props) => {
  //States
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (InternetStatus) {
        ...
        var orderList = getPastDriverOrdersList();
  ...
  const OrderListCallback = (response) => {
    if (Page === 1) {
      ...
      syncPastDriverOrders(response?.data);
      setOrderCount(response?.pagination?.total);
      setSyncOrderCount(getPastDriverOrdersList()?.length);
      ...
      syncPastDriverOrders(NewOrderListList);
      setOrderCount(response?.pagination?.total);
      setSyncOrderCount(getPastDriverOrdersList()?.length);
          ...
          <View style={styles.container}>
            ...
            >
              {InternetStatus && orderCount !== "" ? (
                ...
                >{`${syncOrderCount} past order(s) are synced out of ${orderCount}`}</Text>
              ) : (
                <Text style={styles.textContainer}>{`${
                  getPastDriverOrdersList()?.length
                } past order(s) are cached offline`}</Text>
              )}
              <FlatList
                data={AllOrderList}
...
export default DriverOrdersPastList;
